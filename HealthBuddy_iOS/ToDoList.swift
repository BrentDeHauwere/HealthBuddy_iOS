...
class TodoList {
    ...
    func addItem(item: TodoItem) {
        ...
        notification.userInfo = ["title": item.title, "UUID": item.UUID, "medicineID": "\(item.medicine.id!)", "scheduleID": "\(item.medicalSchedule.id!)"] // assign a unique identifier to the notification so that we can retrieve it later
